...
void LidarCameraFusion::RegisterLidar(const std::string& lidar_dir, bool CUSTOM_MSG) {
  ...
  topics.push_back(std::string("/horizon_lidar/data"));   // pointcloud2
  topics.push_back(std::string("/horizon_lidar/data/")); // livox_ros_driver:customMsg
  ...
  // change pointcloud
  for (size_t i = 0; i < raw_lidar_cloud_->points.size(); ++i) {
    pcl::PointXYZI pt = raw_lidar_cloud_->points[i];
    pt.x = -raw_lidar_cloud_->points[i].y;
    pt.y = -raw_lidar_cloud_->points[i].z;
    pt.z = raw_lidar_cloud_->points[i].x;
    raw_lidar_cloud_->points[i] = pt;
  }

  // save the lidar point cloud
  std::string lidar_save_path = lidar_dir;
  lidar_save_path = lidar_save_path.substr(0, lidar_save_path.find_last_of('.'));
  lidar_save_path += ".pcd";
  std::cout << "Lidar save path: " << lidar_save_path << std::endl;
  pcl::io::savePCDFileBinary(lidar_save_path, *raw_lidar_cloud_);
  in_bag.close();
}
...
cv::Mat LidarCameraFusion::GetProjectionImg(const Vector6d& extrinsic_params) {
  ...
  ProjectLidar2Img(extrinsic_params, raw_lidar_cloud_, DEPTH, false,
                   depth_projection_img, colored_cloud);
